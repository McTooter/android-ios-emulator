...
## 2026-08-26 complete-error audit

The repository is synchronized at `2ab84ff`, and YAML parsing, JSON validation, Python compilation, CMake build, and the native smoke test pass locally. The guest APK harness was hardened to verify package installation, package-path resolution, explicit MAIN/LAUNCHER activity resolution, launcher execution, and process presence; it correctly fails when no ready ADB device exists. Generated Gradle, Python, and native build outputs are now ignored.

The push-triggered runs for `2ab84ff` (`32922912739` Android Test APK and `32922912737` iPadOS Build) again failed in approximately two seconds. Their jobs had no runner name, no steps, and no downloadable logs. The official GitHub billing documentation states that private repositories receive a plan-dependent quota of GitHub-hosted runner minutes and that usage beyond included minutes is billed; GitHub’s workflow troubleshooting documentation specifically recommends reviewing billing errors and setting an Actions budget when billing or storage prevents workflows. Sources: https://docs.github.com/en/actions/concepts/billing-and-usage and https://docs.github.com/en/actions/monitoring-and-troubleshooting-workflows/troubleshooting-workflows.

This confirms that the remaining CI failure cannot be fixed by changing the emulator source or workflow commands. The owner must inspect GitHub account/repository Actions billing, included-minute usage, spending limit, and any payment or account restriction. A self-hosted Mac runner is the alternative for iPadOS/UTM builds; a self-hosted Linux runner could independently run the APK workflow. No more hosted-runner retries should be dispatched until one of those conditions changes.
